...
class VideoFileExplorerActivity : AppCompatActivity() {
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        when (keyCode) {
            KeyEvent.KEYCODE_DPAD_UP -> {
                ...
                val firstVisible =
                    (binding.fileRecyclerView.layoutManager as LinearLayoutManager).findFirstVisibleItemPosition()
                if (firstVisible >= position - 3) {
                    (binding.fileRecyclerView.layoutManager as LinearLayoutManager).scrollToPosition(
                        position - 3
                    )
                }
                return true
            ...
            KeyEvent.KEYCODE_DPAD_DOWN -> {
                ...
                binding.fileRecyclerView.adapter?.notifyItemChanged(position)
                val lastVisible =
                    (binding.fileRecyclerView.layoutManager as LinearLayoutManager).findLastVisibleItemPosition()
            ...
            KeyEvent.KEYCODE_DPAD_CENTER -> {
                if (!isVideo(dataset[getSelected()].file)) {
                    switchFile(dataset[getSelected()].file.absolutePath)
                } else {
                    uriList.clear()
                    val dbService = UriDBService(this)
                    dbService.clearTable()
                    dbService.addUri(Uri.parse(dataset[getSelected()].file.absolutePath))
